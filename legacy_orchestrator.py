...
class Orchestrator:
    ...
    def __init__(self, config: Config):
        ...
        print("Initializing GonKenLab Agent legacy runtime...")
    ...
    def start(self):
        ...
        # Do not speak or advertise the compatibility detector's phrase. The
        # accepted core interaction path will be push-to-talk; wake-word support
        # remains a separately gated extension.
        self._speak(
            "Hello! GonKenLab Agent's legacy source runtime has started."
        )

        # Start wake word detection after greeting finishes
        self.wake_word.start(callback=self._on_wake_word)

        print("GonKenLab Agent legacy runtime is running.")
        print("Its compatibility wake detector is not an accepted core interface.")
    ...
    def _process_query(self, text: str):
        ...
        # Custom action retained only for source-runtime compatibility.
        if "on camera" in text.lower():
            print("[custom] on camera introduction")
            self._speak(
                "Hello, I am GonKenLab Agent, a local AI-lab assistant. "
