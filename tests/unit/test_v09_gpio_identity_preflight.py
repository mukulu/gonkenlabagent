...
class GpioIdentityPreflightTests(unittest.TestCase):
    ...
    def test_probe_deduplicates_duplicate_device_nodes_for_same_canonical_rp1_chip(self):
        rp1=[None]*54
        for bcm in (2,3,17,22,23,27): rp1[bcm]=f"GPIO{bcm}"
        module=FakeGpiod({
            "/dev/gpiochip0":rp1,
            "/dev/gpiochip4":list(rp1),
        }, labels={
            "/dev/gpiochip0":"pinctrl-rp1",
            "/dev/gpiochip4":"pinctrl-rp1",
        })
        with mock.patch("gonken_agent.gpio_resolver._device_identity", return_value="char:254:0:/sys/devices/platform/rp1/gpiochip0"):
            payload=mod.probe((17,22,23,27), gpiod_module=module, chip_paths=("/dev/gpiochip0","/dev/gpiochip4"))
        self.assertEqual(payload["status"],"PASS")
        self.assertEqual(payload["chip_path"],"/dev/gpiochip0")
        self.assertEqual(payload["lines"]["GPIO23"]["resolution_basis"],"unique-line-name")
        self.assertEqual(payload["lines"]["GPIO23"]["canonical_chip_id"],"char:254:0:/sys/devices/platform/rp1/gpiochip0")
        self.assertEqual(payload["lines"]["GPIO23"]["alias_paths"],["/dev/gpiochip0","/dev/gpiochip4"])
