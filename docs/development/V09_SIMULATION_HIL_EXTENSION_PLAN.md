...
The target handoff is deliberately ordered from full simulation to manual unloaded relay verification, then supervised real fan OFF/ON/OFF, then simulated-sensor automatic/semi-automatic HIL. **Checkpoint-23 resolution of this blueprint risk:** the production libgpiod adapter no longer assumes `/dev/gpiochip0` line offset 23 for the BCM23 seed; it resolves a unique kernel line named `GPIO23`, exposes the resolved gpiochip/offset in status, and fails closed when mapping is absent or ambiguous. Target inspection must still establish the actual resolved identity, relay polarity, boot behavior and wiring before actuation.
