...
<!-- MILESTONES -->
Checkpoint scope: **V09 post-checkpoint-34 reliability-first checkpoint replaces the next-candidate boundary with a host/target-shadow release-candidate gate, records the forensic failure chronology/capability matrix/state model/target manifest schema, and starts WP-C by adding canonical gpiochip identity plus alias deduplication before GPIO ambiguity decisions. M10.30 is host-verified; no Raspberry Pi release candidate or physical acceptance is claimed. M10.24 and the real install, reboot/no-login, SHT31, relay/fan, wake/voice, update/rollback/reinstall campaign remain open.**
