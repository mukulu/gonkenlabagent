...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 31 closes the checkpoint-30 Raspberry Pi installation-convergence gaps discovered after activation: stale-release garbage collection is noncritical, I2C reboot is a governed pause/resume transition, Bluetooth installation proves an input route early, and appliance readiness is bound to the exact immutable release. M10.26 is host-verified after regression closure; M10.24 exact-package Raspberry Pi acceptance remains open.**
