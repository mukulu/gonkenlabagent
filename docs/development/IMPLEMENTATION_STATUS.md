...
| M10.39 — V04 component readiness, release identity and environment commissioning foundation | host-verified | not-run | src/gonken_agent/release_identity.py; src/gonken_agent/readiness.py; src/gonken_agent/environment/policy.py; src/gonken_agent/environment/daemon.py; scripts/environment_profile_manager.py; scripts/environment_service_manager.py; scripts/environment_readiness.py; bootstrap.sh; scripts/lib/install_engine.sh; scripts/install.sh; scripts/target_probe.py; scripts/release_readiness.py; tests/fixtures/target_probe/ckpt43_20260917_readiness_identity_mismatch_manifest.json; docs/development/V09_CHECKPOINT_44_V4_FOUNDATION_REPORT.md; docs/development/evidence/v09/ckpt44_focused_component_environment.log; docs/development/evidence/v09/ckpt44_environment_install_integration.log Checkpoint 44 is the host-verified V04 foundation. Next implement phase-aware causal support/journaling, richer environment watch/probe/health, multi-model provisioning/benchmarking/switching and the bounded LLM environment-tool broker. Real SHT31, relay/fan, wake/audio, reboot/no-login, model latency and physical tool transactions require the exact package on the Raspberry Pi. |
| M10.40 — V04 multi-model, typed-tool and causal-evidence convergence | host-verified | not-run | src/gonken_agent/llm/models.py; src/gonken_agent/llm/admin.py; src/gonken_agent/llm/ollama.py; src/gonken_agent/tool_broker.py; src/gonken_agent/component_status.py; src/gonken_agent/voice_runtime.py; src/gonken_agent/support.py; scripts/model_roster_manager.py; packaging/ollama-model-roster.toml; tests/unit/test_v09_multimodel_tools.py; tests/unit/test_v09_llm_admin_components.py; tests/unit/test_v09_model_roster_manager.py; tests/unit/test_support_export.py; docs/development/V09_CHECKPOINT_45_MULTIMODEL_TOOL_CONVERGENCE_REPORT.md; docs/development/evidence/v09/checkpoint45/checkpoint44_target_hybrid_observation.json Run the exact checkpoint-45 package on Raspberry Pi 5 to pull/validate all three admitted models, execute per-model semantic tool and latency/resource benchmarks, exercise local time/date and real-SHT31/simulated-fan voice tools, verify reboot/no-login/model-selection persistence, and only then proceed to separately supervised WP-45C real GPIO23/ELUTENG fan actuation. |
<!-- /MILESTONES -->
