...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 45 completes the dependency-ready V04 host/package convergence tranche after Checkpoint 44: it ingests the successful real-SHT31/simulated-actuator Pi observation, adds the governed three-model roster and switch/benchmark lifecycle, typed LLM tool broker plus deterministic clock/environment fast paths, independent component/operator observability, richer passive environment watch, phase-aware causal one-ZIP evidence, and content-free latency/resource instrumentation. M10.40 is host-verified only after checkpoint-close gates; exact-package three-model/tool target validation, reboot/soak, and supervised real GPIO23/ELUTENG fan acceptance remain open.**
