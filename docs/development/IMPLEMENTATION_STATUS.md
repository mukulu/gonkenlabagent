...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 41 expands target-shadow replay to lifecycle, runtime, resource and model-finalization states across 22 required fixtures. The gate blocks current-release drift, partial installer state, stale temp artifacts, wrong-release support evidence, old managed systemd units, restart failure, low disk, interrupted model finalization and missing operator control-group readiness, while allowing noncurrent corrupt historical releases when the current release is exact. M10.36 is host-verified as replay/readiness evidence only; no Raspberry Pi release candidate or physical acceptance is claimed. Sanitized real-target manifests, remaining host/target-shadow release-candidate gating and the M10.24 hardware/voice/lifecycle campaign remain open.**
