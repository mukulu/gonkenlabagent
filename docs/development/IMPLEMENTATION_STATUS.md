...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 43 completes the host/target-shadow reliability-first convergence cycle triggered by the 2026-09-17 checkpoint-42 target failure. It repairs and target-shadows service-context audio capture, freshness-binds semantic appliance readiness, consolidates installer-failure plus canonical support evidence into one operator-accessible ZIP, hardens launcher/bootstrap/installer ownership boundaries, reruns downstream voice/speech/Ollama/environment and lifecycle regression portfolios, and passes an exact tagged pre-final archive qualification. M10.38 is host-verified; M10.24 real Raspberry Pi physical audio, SHT31, relay/fan, wake, reboot/no-login and lifecycle acceptance remains not-run for this exact package.**
