...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 32 repairs the checkpoint-31 Raspberry Pi immutable-release self-mutation defect and hardens current-release-only installation semantics: all runtime smoke completes before sealing, transient interpreter caches are purged before the digest is recorded, sealed releases use static validation only, runtime binding checks are a separate current-release gate, and historical releases are not executed/revalidated during normal install/runtime. M10.27 is host-verified; M10.24 physical acceptance remains open.**
...
| M10.27 — Current-release immutable seal and post-seal runtime isolation repair | host-verified | not-run | scripts/release_manager.py; scripts/install.sh; scripts/ci.sh; tests/unit/test_m3_3_release_manager.py; tests/unit/test_v09_install_dependency_graph.py; tests/integration/test_release_lifecycle_process.py; tests/integration/test_speech_lifecycle_process.py; docs/development/evidence/v09/checkpoint32/target_incident_summary.json; docs/development/evidence/v09/checkpoint32/final_unit_accounting.json; docs/development/evidence/v09/checkpoint32/final_integration_accounting.json; docs/development/evidence/v09/checkpoint32/final_release_lifecycle_accounting.json; docs/development/V09_CHECKPOINT_32_REPORT.md Retest exact checkpoint 32 on the Raspberry Pi. Normal install/runtime must operate on the current checkpoint only: runtime smoke/API checks occur before sealing, activation uses static current-candidate integrity checks, runtime binding capability is checked later against the current release, and historical releases are not executed/revalidated except during an explicit operator rollback. INSTALLATION_COMPLETE and remaining M10.24 physical evidence are still required. |
<!-- /MILESTONES -->
