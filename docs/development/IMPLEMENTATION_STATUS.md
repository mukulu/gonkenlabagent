...
| M10.9 — Simulation foundations | host-verified | not-run | TEST_MATRIX.md (Checkpoint 17); tests/unit/test_v09_environment_simulation.py; affected environment/config/IPC/daemon/CLI test slice Operator env simulate CLI, passive watch, diagnostics/support simulation visibility, hybrid HIL and physical M10.7 acceptance remain open. |
| M10.10 — Operator simulation experience | pending | not-run | docs/development/V09_SIMULATION_HIL_EXTENSION_PLAN.md Implement env simulate CLI family, passive watch using state.snapshot.get, simulation fault injection UX, diagnostics/support visibility and full-simulation acceptance tests. |
