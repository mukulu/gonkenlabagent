...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 42 makes one comprehensive target evidence ZIP the required handoff unit for future build cycles. Normal support export and installer-failure bundles now include a sanitized non-actuating target manifest, platform/resource inventory, bounded service-event codes, installer provenance, preflight summaries and an evidence index while preserving content-free privacy boundaries. M10.37 is host-verified as support/failure-bundle evidence infrastructure only; the platform still does not directly run physical hardware, and M10.24 physical observations remain target-collected evidence carried by the ZIP.**
