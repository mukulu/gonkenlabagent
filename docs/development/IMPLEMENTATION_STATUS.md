...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 38 expands target-shadow replay beyond GPIO identity into explicit opt-in checks for duplex audio capability, service identity and release-state cleanliness. Release readiness now requires six replay fixtures: legacy GPIO alias/duplicate coverage plus full capability PASS, ambiguous audio expected FAIL, missing service identity expected FAIL and dirty release-state expected FAIL. M10.33 is host-verified as internal replay evidence only; no Raspberry Pi release candidate or physical acceptance is claimed. Real target manifest expansion, exact archive qualification and the M10.24 hardware/voice/lifecycle campaign remain open.**
