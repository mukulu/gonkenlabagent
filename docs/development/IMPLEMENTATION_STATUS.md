...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 30 repairs the checkpoint-29 Raspberry Pi activation migration defect; M10.25 is host-verified, while M10.24 exact-package Raspberry Pi acceptance remains open after partial actuator evidence and a blocked checkpoint-29 install.**
...
| M10.24 — Final Raspberry Pi environment and release acceptance | host-verified | not-run | docs/development/V09_COMPREHENSIVE_CLOSURE_BLUEPRINT_CHECKPOINT_25.md; docs/RASPBERRY_PI_ACCEPTANCE_RUN.md; checkpoint-29 target transcript: GPIO23 gpiochip0:23 manual 0/OFF, 1/ON, 0/OFF physical fan response; checkpoint-29 installer reached target preflight/build then BLOCKED at activate_release with RELEASE_BINDING_MANIFEST while reconciling a pre-bridge active release Partial target evidence is preserved: the Raspberry Pi 5 GPIO23/relay/PENGLIN/ELUTENG load path works manually. Full M10.24 remains NOT_RUN because checkpoint 29 did not reach INSTALLATION_COMPLETE. Retest exact checkpoint 30; then complete GonKen CLI actuator, SHT31, full-real controller, voice/wake, faults, reboot/no-login, update/rollback/reinstall and final support evidence. |
