...
<!-- MILESTONES -->
Checkpoint scope: **V09 checkpoint 44 begins the V04 full-component-convergence cycle from exact checkpoint 43. It fixes immutable runtime readiness identity, introduces component-readiness v2 foundations, makes environment checks non-mutating, integrates canonical environment profile/address selection into bootstrap/source-record/install, reconciles known environment ownership/drop-in/restart drift, requires semantic sensor/backend readiness for non-actuating commissioned profiles, and adds the checkpoint-43 readiness-identity target-shadow regression. M10.39 is host-verified only after checkpoint-close gates; multi-model/tool-broker expansion and M10.24 physical Pi acceptance remain open.**
