...
<!-- MILESTONES -->
Checkpoint scope: **Continuation 07: target diagnostic snapshots, support upload workflow and end-user onboarding**
...
| M9.1 — Clean-install and failure campaign | partial | not-run | TEST_MATRIX.md continuation-07; README.md; scripts/collect-support.sh; src/gonken_agent/diagnostics.py Cloud cannot perform physical clean-image acceptance. The repository now records startup hardware/software evidence and support ZIPs for uploaded target logs; the actual clean-install, reboot, power-loss and hardware campaign remains target-run work. |
| M9.2 — Security/license review | partial | not-run | TEST_MATRIX.md continuation-07; CONTINUATION_01_REPORT.md; D-050/D-051, D-060-D-064 and D-072 New-code review, inherited identity invariant, privacy tests, privilege-pattern service checks, uninstall protection tests, credential-pattern scan and content-free diagnostic snapshot tests pass. Target hardening, artifact licenses and redistribution approval remain open. |
| M9.3 — Documentation and onboarding | host-verified | not-run | README.md; scripts/collect-support.sh; TEST_MATRIX.md continuation-07; docs/development/IMPLEMENTATION_STATUS.md README now begins with end-user Raspberry Pi OS, SSH, bootstrap, service check, log/support export, update/rollback/uninstall and configuration guidance. Target screenshots/operator feedback remain future refinements after a real Pi run. |
...
| M9.5 — Portable Git handoff | host-verified | not-applicable | CONTINUATION_01_REPORT.md; checkpoint/continuation-01 through checkpoint/continuation-06 Private checkpoint transport has been repeatedly verified through ZIP integrity, extracted branch/tag/HEAD, executable modes, clean Git and strict object checks. Continuation-07 checkpoint pending until this diagnostic/onboarding work is committed and packaged. |
<!-- /MILESTONES -->
