...
| M10.17 — Target Python dependency boundary redesign | pending | not-run |  Select and verify a controlled target binding architecture; reproduce checkpoint-24 dirty-system-package contamination and convert it into regression protection. |
| M10.18 — Installer convergence DAG, preflight and failure-evidence hardening | pending | not-run |  Implement prerequisite/postcondition convergence and dirty/partial/interrupted-state regression coverage. |
| M10.19 — Accounts, groups, configuration and environment-profile convergence | pending | not-run |  Implement least-privilege identity/ownership/profile convergence for simulation, both hybrid modes and full-real. |
