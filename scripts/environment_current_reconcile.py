...
def _inputs(root: Path, release_root: Path, candidate: str, mode: str) -> dict:
    ...
    return {'candidate_commit': candidate, 'current_commit': releases.current_commit(release_root),
            'configuration_inputs_sha256': digest.hexdigest(), 'mode_requested': mode}
...
def reconcile(root: Path, release_root: Path, state_root: Path, candidate: str,
    ...
    identity = _inputs(root, release_root, candidate, mode)
    if check:
        ...
        if record['status'] == 'READY_EXISTING_ENVIRONMENT':
            ...
            policy.converge(agent, mode, check=True)
        return record
    ...
    if identity['current_commit'] is not None:
        try:
            ...
            selected_policy = policy.converge(agent, mode)
            if _inputs(root, release_root, candidate, mode) != identity:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        result = reconcile(*paths[:3], args.candidate_commit, args.mode, *paths[3:], check=args.check)
